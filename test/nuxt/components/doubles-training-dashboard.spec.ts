...
import DoublesTrainingDashboard from '~/components/doubles-training-dashboard.vue'

const createGame = (
  id: string,
  createdAt: string,
  hitPercent: number,
): Serialized<DoublesTrainingGame> => ({
  id,
  createdAt,
  updatedAt: createdAt,
  hitPercent,
...
const props: GameData<Serialized<DoublesTrainingGame>> = {
  type: 'doublesTraining',
  range: 'allTime',
  games: [
...
describe('DoublesTrainingDashboard', () => {
  it('should render correctly with default props', async () => {
    const component = await mountSuspended(DoublesTrainingDashboard, { props })

    expect(component.html()).toMatchSnapshot()
  })

  it('should show empty state when no games are provided', async () => {
    const component = await mountSuspended(DoublesTrainingDashboard, {
      props: { ...props, games: [] },
    })
